  ...
  rows.forEach((row, index) => {
    ...
    if (isDeleteRow(row)) {
      ...
      if (!row.nom_club) {
        ...
        && !presentOptionalColumns.some((column) => String(row[column] ?? '').trim())
      ) {
        issues.push({
          type: 'error',
          row: rowNumber,
          field: presentOptionalColumns[0],
          message: `Renseignez au moins une colonne optionnelle à la ligne ${rowNumber}.`,
        });
      }
    }
    ...
    .map((club) => {
      ...
      FIELD_MAPPINGS.forEach(({ source, target }) => {
        if (importedColumns.includes(source)) {
          nextClub[target] = importedRow[source] ?? '';
        }
      });
