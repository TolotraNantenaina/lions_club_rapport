...
import { getFileExtension } from './slugify';
import { optimizeLogoBuffer } from './optimizeLogo.mjs';
import { ALLOWED_LOGO_EXTENSIONS, buildLogoFileName, MAX_LOGO_SIZE_BYTES } from './validateLogo';
...
export function sanitizeLogoFileName(nomClub, typeClub, extension) {
  return buildLogoFileName(nomClub, typeClub, extension);
}
...
export async function saveClubLogo(file, club) {
  ...
  // Redimensionnement avant écriture : les logos servaient jusqu'ici des
  // originaux d'appareil photo (jusqu'à 7 Mo) pour un rendu en 155 px.
  // Le format est conservé, seules les dimensions changent.
  let optimizedBuffer;

  try {
    const optimized = await optimizeLogoBuffer(buffer, extension);
    optimizedBuffer = optimized.buffer;
  } catch (error) {
    throw new Error(
      'L’image n’a pas pu être traitée. Elle est peut-être corrompue ou d’une définition excessive.',
    );
  }

  await mkdir(CLUB_LOGOS_DIR, { recursive: true });

  const fileName = buildLogoFileName(club.nomClub, club.typeClub, extension);
