...
import { normalizeImportedRows, REQUIRED_XLSX_COLUMNS, normalizeHeader } from './validateClubData';
...
export async function parseExcelFile(file) {
  ...
  try {
    ...
    const headers = Array.isArray(headerRows[0])
      ...
      : [];
    const missingColumns = REQUIRED_XLSX_COLUMNS.filter((column) => !headers.includes(column));
    ...
    if (missingColumns.length > 0) {
      return {
        ...
        issues: [{
          type: 'error',
          message: `Colonnes manquantes : ${missingColumns.join(', ')}. Colonnes attendues : ${REQUIRED_XLSX_COLUMNS.join(', ')}.`,
        }],
      };
    }
