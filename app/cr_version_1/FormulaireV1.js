 ...
 * textarea is replaced by a TipTap rich-text editor (DiversEditor).
 *
 * @param {{
 *   data: object,
 ...
 * }} props
 */
export default function FormulaireV1({ data, onChange, clubsData = [], clubsLoading = false, clubsError = '', showToast }) {
      ...
      <div className="mx-auto grid gap-8">
          ...
          <section className="space-y-6 rounded-[12px] bg-white/95 p-8 shadow-[0_20px_60px_rgba(0,0,0,0.12)]">
            ...
            <div className="grid gap-6 sm:grid-cols-2">
              ...
              <FormField label="Membre TOTAL" htmlFor="memberTotal" className="sm:col-span-2">
                <input id="memberTotal" type="number" min="0" value={formData.memberTotal} readOnly className="form-input bg-slate-100 pt-[12px]" />
              </FormField>
            </div>
          </section>
        </div>

        {/* ═══ Section: Interventions générales ═══════════════ */}
        <div className="grid gap-8 min-[1200px]:min-w-[990px] min-[1200px]:mx-auto">
          <section className="space-y-6 rounded-[12px] bg-white/95 p-8 shadow-[0_20px_60px_rgba(0,0,0,0.12)]">
        ...
        <div className="grid gap-8 min-[1200px]:min-w-[990px] min-[1200px]:mx-auto">
          <section className="space-y-6 rounded-[12px] bg-white/95 p-8 shadow-[0_20px_60px_rgba(0,0,0,0.12)]">
            ...
            <label htmlFor="miscellaneous" className="space-y-2 block">
              <span className="text-[0.95em] font-semibold text-primary mb-[8px]">Divers</span>
              <DiversEditor
                content={formData.miscellaneous || ''}
                onUpdate={(html) => updateField('miscellaneous', html)}
                showToast={showToast}
              />
            </label>
          </section>
