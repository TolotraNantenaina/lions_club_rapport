    ...
    <div className={`${sticky ? 'sticky top-0 z-20 rounded-t-lg border-t border-slate-200' : ''} flex flex-wrap items-center gap-1 bg-slate-50 px-3 pt-2 pb-2`}>
      {hidden && (
        <button type="button" onClick={onToggleHide} title="Afficher la barre d'outils"
          ...
          <svg viewBox="0 0 24 24" className="h-3.5 w-3.5" fill="none" stroke="currentColor" strokeWidth="2"><path d="M2 12s3-7 10-7 10 7 10 7-3 7-10 7S2 12 2 12z"/><circle cx="12" cy="12" r="3"/></svg>
          Outils
        </button>
      )}
      ...
      <Group label="Listes">
        ...
        <TBtn active={isActive('orderedList')} onClick={() => run(() => editor.chain().focus().toggleOrderedList().run())} title="Liste numérotée">
          <svg viewBox="0 0 24 24" className="h-4 w-4" fill="none" stroke="currentColor" strokeWidth="2"><path d="M10 6h11M10 12h11M10 18h11M4 6V2l-1 1M3 10h2M3 14h2M4 18l-1 1"/></svg>
        </TBtn>
      ...
      <Group label="Alignement">
        <TBtn active={isActive({ textAlign: 'left' })} onClick={() => run(() => editor.chain().focus().setTextAlign('left').run())} title="Gauche">
          <svg viewBox="0 0 24 24" className="h-4 w-4" fill="none" stroke="currentColor" strokeWidth="2"><path d="M3 6h18M3 12h12M3 18h16"/></svg>
        </TBtn>
        <TBtn active={isActive({ textAlign: 'center' })} onClick={() => run(() => editor.chain().focus().setTextAlign('center').run())} title="Centré">
          <svg viewBox="0 0 24 24" className="h-4 w-4" fill="none" stroke="currentColor" strokeWidth="2"><path d="M3 6h18M6 12h12M4 18h16"/></svg>
        </TBtn>
        <TBtn active={isActive({ textAlign: 'right' })} onClick={() => run(() => editor.chain().focus().setTextAlign('right').run())} title="Droite">
          <svg viewBox="0 0 24 24" className="h-4 w-4" fill="none" stroke="currentColor" strokeWidth="2"><path d="M3 6h18M9 12h12M5 18h16"/></svg>
        </TBtn>
        <TBtn active={isActive({ textAlign: 'justify' })} onClick={() => run(() => editor.chain().focus().setTextAlign('justify').run())} title="Justifié">
          <svg viewBox="0 0 24 24" className="h-4 w-4" fill="none" stroke="currentColor" strokeWidth="2"><path d="M3 6h18M3 12h18M3 18h18"/></svg>
        </TBtn>
      </Group>
      ...
      <div className="relative" ref={colorRef}>
        <TBtn active={showColors} onClick={() => { setShowColors(!showColors); setShowTableGrid(false); }} title="Couleur du texte">
          <div className="flex flex-col items-center">
            ...
            <div className="h-1 w-3 rounded-sm bg-[#c0392b]" />
          </div>
        </TBtn>
        {showColors && (
          <div className="absolute left-0 top-full z-50 mt-1 flex gap-1.5 rounded-lg border border-slate-200 bg-white p-2 shadow-xl">
            {COLORS.map((c) => (
              <button key={c} type="button"
                ...
                className="h-5 w-5 rounded-full border border-slate-200 transition hover:scale-125"
      ...
      {/* ── Import ────────────────────────────────────── */}
      <TBtn onClick={() => document.getElementById(fileInputId)?.click()} title="Importer un fichier">
      ...
      {/* ── Hide button ───────────────────────────────── */}
      <TBtn onClick={onToggleHide} title="Masquer la barre d'outils">
